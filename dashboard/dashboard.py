...
def get_inputfield(request):
    try:
        ...
        qry1 = "select input_fields,input_type,id_name from upload.Web_inputfields where task_type=" + str(task_type) + " and status = 1"
        qry2 = "select name from upload.Web_tasklist where tasklist_pk=" + str(task_type)
            ...
            for row in result:
                ...
                if row[1] == 'dropdown':
                    if row[2] == 'repeat':
                        ...
                        view_data['dp'] = get_emp_details()['emp_details_lst']
                    elif row[2] =='assignee':
                        view_data['dp'] = get_emp_details()['emp_details_lst']
    ...
    try:
        ...
        if 'assignee_fk' in keys and 'checker_fk' in keys:
            if str(values[keys.index('assignee_fk')]) == str(values[keys.index('checker_fk')]):
                ...
                data = ','.join(values)
                qry = ("insert into upload.Web_task_assignment(comp_fk,assigner_fk,taskupload_data," + ','.join(
            ...
            data = ','.join(values)
            qry = ("insert into upload.Web_task_assignment(comp_fk,assigner_fk,taskupload_data," + ','.join(keys) + ",dt,status) values(?,?,?," + data + ",getdate(),?)")
            values = (comp_fk,assigner_fk,str(taskdata),1)
            py_connection.put_result(qry, values)
            return {"message": "Task Created Successfully", "rval": 1}
    except Exception as e:
...
def fn_get_taskdata(request, decoded):
    try:
        ...
        comp_fk = decoded.get("comp_fk")
        sql_command = "{call upload.Web_sp_gettaskdata(?,?,?,?)}"
...
def fn_get_adminreport(request, decoded):
    try:
        ...
        comp_fk = decoded.get("comp_fk")
        assigner_fk = decoded.get("emp_fk")
        sql_command = "{call upload.Web_sp_adminreport(?,?,?,?,?)}"
...
def fn_get_employeereport(request, decoded):
    try:
        ...
        comp_fk = decoded.get("comp_fk")
        sql_command = "{call upload.Web_sp_employeereport(?,?,?)}"
...
def fn_get_checkerupdatestatus(request):
    try:
        ...
        sqlcommands = "{call upload.Web_sp_checkerstatusupdate(?,?,?)}"
...
def fn_get_web_taskcount(decoded):
    try:
        comp_fk = decoded.get("comp_fk")
        emp_fk = decoded.get("emp_fk")
        sql_command = "{CALL upload.Web_sp_taskcount(?, ?)}"
...
def fn_get_admin_taskcount(decoded):
    try:
        ...
        sql_command = "{CALL upload.Web_sp_admintaskcount(?, ?)}"
        params = (emp_fk, comp_fk)
        result, k = py_connection.call_prop1(sql_command, params)
        lst = []
...
def fn_get_checker(request, decoded):
    try:
        ...
        employee_fk = decoded.get("emp_fk")
        comp_fk = decoded.get("comp_fk")
        sql_command = "{call upload.Web_sp_checker(?,?,?,?)}"
...
def get_encoding_filestring(request, decoded):
    try:
        ...
        qry = "select doc_path, access_key from upload.web_doc_upload where doc_pk =" + str(doc_fk) + " and comp_fk=" + str(comp_fk)
...
def get_reportfile(request, decoded):
    try:
        task_fk = request.get("task_fk")
        qry = "SELECT doc_path,access_key,document_pk FROM upload.Web_v_assigned_task WHERE doc_pk =" + str(task_fk)
        res = py_connection.get_result(qry)
