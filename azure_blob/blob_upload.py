...
def blob_connection(decoded):
    try:
        ...
        qry = "SELECT connection_string, container_name FROM upload.web_v_organization where comp_pk=" + str(comp_fk)
...
def updatefilereadlog(read_size, decoded, doc_fk):
    ...
    qry = "insert into upload.Web_file_readlog(read_size, comp_fk, emp_fk, doc_fk, dt) values(?,?,?,?,?)"
